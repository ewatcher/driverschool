...
<script type="text/javascript" charset="utf-8">
	...
	function initUpdateArrangeDetailDatagrid(detailDialog,trainerId,trainerName,schoolArea,arrangeType){
		var updateDatagridId=detailDialog.find('table[id=demo_trainerArrangedetail_datagrid]');
			...
			columns:[[{
				title : '<font color="#0099FF">7:00-8:00</font>',
				...
				title : '<font color="#0099FF">field7</font>',
			...
			onBeforeLoad:function(param){
				var btnAdd=detailDialog.find('a[id=add-btn-toolbar]');
				var btnUpdate=detailDialog.find('a[id=edit-btn-toolbar]');
				var btnDel=detailDialog.find('a[id=del-btn-toolbar]');
	...
	function arrangeDetailDatagridLoadSuccess(data){
		...
		arrangeDetailDatagrid.datagrid('hideColumn','reservationFieldId7');
	...
	function edit(arrangeDetailDatagrid,trainerId,trainerName,schoolArea) {
		var itemTime7flagTemp=undefined;
		...
		if (rows.length == 1) {
			...
			var p = parent.sy.dialog({
									...
									handler : function() {
										...
										$.ajax({
											...
											data : {trainerReservationDetailId:rows[0].trainerReservationDetailId,
													...
													item7TrainContent:itemTime7flagTemp.combobox('getValue'),
						...
						onLoad : function() {
							...
							var f = p.find('form');
							var itemTime7Temp=f.find('input[name=ItemTime7]');
							...
							itemTime7flagTemp=f.find('input[name=item7TrainContent]');
						...
						initComboboxData(itemTime7flagTemp,rows[0].item7TrainContent,workingTypeUrl,true);
						...
						//如果该时段存在预约不允许修改排班
						disabledUpdateItemtime(itemTime7flagTemp,itemTime7Temp,rows[0].reservationFieldId7);
						...
						//初始化时间段选择
						if(rows[0].item7TrainContent!=5){
							itemTime7Temp.attr('checked',true);
						}
						...
						//如果不选中该时间，则排班标志显示为休息
						initItemtimeFlag(itemTime7Temp,itemTime7flagTemp,rows[0].itemTime7flag);
