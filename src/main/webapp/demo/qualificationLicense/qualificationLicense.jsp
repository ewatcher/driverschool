...
<script type="text/javascript" charset="utf-8">
	...
	$(function() {
		...
		mydatagrid = $('#demo_qualificationLicense_datagrid').datagrid({
			url : '${pageContext.request.contextPath}/demo/qualificationLicenseAction!datagrid.action',/* 从后台读取所有user数据 */
			title : '资格证信息管理列表!', /* 表头标题 */
			...
			frozenColumns : [ [{
				...
				title : '<font color="#0099FF">资格证证号</font>',
			...
			onRowContextMenu : function(e, rowIndex, rowData) {
				...
				$('#demo_qualificationLicense_menu').menu('show', {
					left : e.pageX,
					top : e.pageY
				});
			},
	...
	function onLoadSuccess(data) {
		...
		//初始化查询相关属性选项值
		//初始化校区选项
		initSchoolArea($("#demo_qualificationLicense_organization"),userSchoolArea);
		//初始化驾照类型选项
		$("#demo_qualificationLicense_driverType").combobox({
			...
			method:'get',
			onChange:function(){
				//根据combobox选择名称检索资格证学员信息
				mydatagrid.datagrid('load', sy.serializeObject($("#demo_qualificationLicense_searchForm")));
			}
		});
		...
		if(data.total==0){
			mydatagrid.datagrid('appendRow', {
				studentOrganization: '<div style="text-align:center;color:red">没有资格证记录信息！</div>' })
	...
	function edit() {
		...
		if (rows.length == 1) {
			var p = parent.sy.dialog({
						title : '编辑资格证信息',
						href : '${pageContext.request.contextPath}/demo/qualificationLicenseAction!toQualificationLicenseEditPage.action',
						width : 1024,
						height : 520,
						buttons : [
								{
									...
									handler : function() {
										...
										//提交表单
										f.form('submit',{
											url : '${pageContext.request.contextPath}/demo/qualificationLicenseAction!edit.action',
											success : function(d) {
						...
						onLoad : function() {
							...
							//将学员基本信息属性襟止修改
							var inputDisable=f.find('input[class=demo_qualificationLicense_update]');
	...
	function read() {
		...
		if (rows.length == 1) {
			var p = parent.sy.dialog({
						title : '----> 资格证管理详细信息  <----',
						href : '${pageContext.request.contextPath}/demo/qualificationLicenseAction!toQualificationLicenseDetailPage.action',
						width : 1024,
						height : 520,
						buttons : [
								{
	...
	function append() {
		var addDialog = parent.sy.dialog({
					title : '资格证信息录入',
					href : '${pageContext.request.contextPath}/demo/qualificationLicenseAction!toQualificationLicenseAddPage.action',
					width : 1024,
					height : 585,
					buttons : [
							 {
							...
							onLoad : function() {
								...
								var myLinkButton_search=myForm.find('a[id=qualificationLicense_a_searche]');
								var myLinkButton_cancel=myForm.find('a[id=qualificationLicense_a_cancel]');
								var myDatagird=myForm.find('table[id=demo_qualificationLicense_selectStudent]');
								...
								//初始化添加页面数据
								if(userSchoolArea!==''){
									initQualificationLicense(addDialog,myDatagird,userSchoolArea);
								}else{
									initQualificationLicense(addDialog,myDatagird,null);
								}
								...
								//取消按钮事件
								var resetBtn=myForm.find('input[class=demo_add_cancle]');
	...
	//资格证添加输入页面学员信息列表
	function initQualificationLicense(addDialog,myDatagird,schoolArea){
		// 展现符合资格证的学员信息
		myDatagird.datagrid({
			url : '${pageContext.request.contextPath}/demo/studentExamAction!datagrid.action',
			queryParams:{qualificationFlag:1,qualificationFinishFlag:0,schoolArea:schoolArea},
			pagination : true, 
			pagePosition : 'bottom', 
			pageSize : 20, 
	...
	function addQuitRecord(rows){
		...
		var addInputDialog=parent.sy.dialog({
			...
			title : '资格证信息录入',
			href : '${pageContext.request.contextPath}/demo/qualificationLicenseAction!toQualificationLicenseAddInputPage.action',
			width :1024,
			height : 600,
			buttons : [
				{
					...
					handler : function() {
						...
						f.form('submit',{
							url : '${pageContext.request.contextPath}/demo/qualificationLicenseAction!add.action',
							success : function(d) {
								var json = $.parseJSON(d);
								if (json.success) {
			...
			onLoad : function() {
				...
				var f = addInputDialog.find('form');
				var disableItems=f.find('input[class=demo_qualificationLicense_update]');
			...
			parent.sy.messagerConfirm(
							...
							function(r) {
								if (r) {
									...
									$.ajax({
										url : '${pageContext.request.contextPath}/demo/qualificationLicenseAction!delete.action',
										data : {ids : ids.join(',')},
										dataType : 'json',
										success : function(d) {
	...
	function cleanSearch() {
		mydatagrid.datagrid('load', {});
		//清空与查询有关的输入框
		$('#demo_qualificationLicense_searchForm input').val('');
	}
...
<body class="easyui-layout" data-options="fit:true">
	<div data-options="region:'north',border:false,title:'查询功能'" style="height: 125px;overflow: hidden;" align="left">
		<!-- 人员信息查询表单 -->
		<form id="demo_qualificationLicense_searchForm">
			<table class="tableForm datagrid-toolbar" style="width: 100%;height: 100%;">
				<tr>
				<th style="width: 65px;">所属校区:</th>
				<td><input id="demo_qualificationLicense_organization" name="schoolArea" style="width: 155px;"/></td>
				<th style="width: 50px;">编号:</th>
				<td><input id="demo_theory_driverType" name="numbering" style="width: 155px;"/></td>
				<th>资格证发放日期</th>
				<td>
			...
			<tr>
				...
				<th style="width: 65px;">资格证号:</th>
				<td><input name="studentIdentityId"  style="width: 155px;" /></td>
				<th style="width: 65px;">驾照类型:</th>
				<td><input id="demo_qualificationLicense_driverType" name="driverType" style="width: 155px;" /></td>
				...
				<tr>
					...
					<td></td>
					<td colspan="2">
						<a href="javascript:void(0);" id="qualificationLicense_a_searche" class="easyui-linkbutton" iconCls="icon-search" onclick="mySearch();">查询</a>
						<a href="javascript:void(0);" id="qualificationLicense_a_canel" class="easyui-linkbutton" iconCls="icon-cancel" onclick="cleanSearch();">取消</a>
					</td>
					<th></th>
					<td></td>
				</tr>
	...
	<div data-options="region:'center',border:false"
		...
		<table id="demo_qualificationLicense_datagrid"></table>
	</div>
	<!-- 操作按钮事件 -->
	<div id="demo_qualificationLicense_menu" class="easyui-menu" style="width:120px;display: none;">
