...
<script type="text/javascript" charset="utf-8">
	...
	var trainingItemJson=[{"value":1,"text":"自动档"},{"value":2,"text":"三塘"},{"value":3,"text":"五项"},{"value":4,"text":"路训"},{"value":5,"text":"休息"},{"value":null,"text":"取消"}];
	...
	$(function() {
		...
		mydatagrid = $('#demo_trainerreservation_datagrid').datagrid({
			url : '${pageContext.request.contextPath}/demo/trainerReservationAction!datagrid.action',
	...
	function reservationDetail() {
		...
		if (rows.length > 0) {
			reservationDetailDialog=parent.sy.dialog({
				title : '教练预约明细信息',
				href : '${pageContext.request.contextPath}/demo/reservationDetailAction!toTrainerReservationDetailPage.action',
				width : 1280,
				height : 785,
				buttons : [
						{
				...
				onLoad : function() {
					...
					//初始化月份查询选项
					var startDate=f.find('input[name=reservationDateBegin]');
					startDate.datebox({
					});
					var endDate=f.find('input[name=reservationDateEnd]');
					endDate.datebox({
					});
	...
	function initReservationDetailDatagrid(trainerId,studentNoEditer){
		...
		reservationDetailDatagrid=datagridId.datagrid({
			url:'${pageContext.request.contextPath}/demo/reservationDetailAction!datagrid.action',
	...
	function onLoadSuccessReservationDetial(data){
		...
		if(data.total==0){
			...
			.datagrid('mergeCells', { index: 0, field: 'reservationDate', colspan:4 });
		}
	...
	<div data-options="region:'north',border:false,title:'查询功能'" style="height: 85px;overflow: hidden;" align="left">
		...
		<form id="demo_trainerreservation_searchForm">
			...
			<tr>
				...
				<td><input  name="trainerName" class="keypress_event" style="width: 155px;"/></td>
				<th style="width: 75px;">身份证:</th>
				<td><input name="trainerIdentityId" class="keypress_event" style="width: 155px;" /></td>
			</tr>
			<tr>
				<th></th>
				<td></td>
