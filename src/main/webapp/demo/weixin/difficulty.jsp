...
<body class="easyui-layout" data-options="fit:true">
    ...
    <div data-options="region:'center',border:false" style="overflow: hidden;">
	  <table id="difficultyDataGrid" class="easyui-datagrid" title="学员难点反馈信息列表" 
			data-options="rownumbers:true,
			fit:true,
			pagination:true,
			singleSelect:true,
			url:'${pageContext.request.contextPath}/demo/difficultyAction!datagrid.action',
			method:'get',
			toolbar:toolbar,
		 ...
		 <thead>
			<tr>
                <th data-options="field:'organizationName',align:'center'" width="10%">所属机构</th>
				<th data-options="field:'nickname',align:'center'" width="10%">微信昵称</th>
				<th data-options="field:'createTime',align:'center'" width="10%">提交时间</th>
				<th data-options="field:'difficulty',formatter:function(val, rows, index){
                val = val.replace(/\n/g, '<br />');
                ...
                var str = '<p style=width:100%;height:auto;text-align:left;white-space:normal;margin-right:25px; >' + val + '</p>';
                return str;
              },editor:'textbox'" width="38%">备注</th>
			</tr>
		 </thead>
	  </table>
    </div>
	<script type="text/javascript">
		var mydatagrid = $('#difficultyDataGrid');
		var userACLs=[];
		var toolbar = [{
			id:'edit-btn-toolbar',
			text:'修改',
			...
			if (endEditing()){
				if (clickIndex != undefined && mydatagrid.datagrid('validateRow', clickIndex)){
					...
					$.messager.confirm('Confirm','您确定需要删除该条数据吗?',function(r){
					    if (r){
					    	saveSubmit('${pageContext.request.contextPath}/demo/difficultyAction!delete.action', {'ids':row.id}); // 删除数据
					    }
					});
				} else {
			...
			if (endEditing()){
				//var insertRows = mydatagrid.datagrid('getChanges','inserted'); // 得到新增的数据
   				...
   				if(updateRows.length > 0) {
   					saveSubmit('${pageContext.request.contextPath}/demo/difficultyAction!edit.action', updateRows[0]);
   				} 
   			}
			enableButton(); // 开启功能按钮
		}
