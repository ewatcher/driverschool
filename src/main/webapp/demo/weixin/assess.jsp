...
<body class="easyui-layout" data-options="fit:true">
      ...
      <form id="menu_searchForm">
        <table class="tableForm datagrid-toolbar" style="width: 100%;height: 100%;">
          <tr>
				...
				<th style="width: 65px;">教练名称:</th>
				<td><input id="demo_trainerName" class="reservation_keyEvent" name="trainerName" style="width: 150px;"/></td>
    ...
    <div data-options="region:'center',border:false" style="overflow: hidden;">
	  <table id="assessDataGrid" class="easyui-datagrid" title="教练评价管理" 
			data-options="rownumbers:true,
			fit:true,
			pagination:true,
			singleSelect:true,
			url:'${pageContext.request.contextPath}/demo/assessAction!datagrid.action',
			method:'get',
			toolbar:toolbar,
		 ...
		 <thead>
			<tr>
                ...
                <th data-options="field:'trainerName',align:'center'" width="4%">教练名称</th>
                 <th data-options="field:'serviceMark',align:'center'," width="4%">服务态度</th>
                  <th data-options="field:'efficientMark',align:'center'," width="4%">办事效率</th>
                   <th data-options="field:'dutyMark',align:'center'," width="4%">责任感</th>
                    <th data-options="field:'groomingMark',align:'center'," width="4%">仪容仪表</th>
                <th data-options="field:'totalMark',align:'center'," width="8%">总分</th>
			...
			if (endEditing()){
				if (clickIndex != undefined && mydatagrid.datagrid('validateRow', clickIndex)){
					...
					$.messager.confirm('Confirm','您确定需要删除该条数据吗?',function(r){
					    if (r){
					    	saveSubmit('${pageContext.request.contextPath}/demo/assessAction!delete.action', {'ids':row.id}); // 删除数据
					    }
					});
				} else {
			...
			if (endEditing()){
				//var insertRows = mydatagrid.datagrid('getChanges','inserted'); // 得到新增的数据
   				...
   				if(updateRows.length > 0) {
   					saveSubmit('${pageContext.request.contextPath}/demo/assessAction!edit.action', updateRows[0]);
   				} 
   			}
			enableButton(); // 开启功能按钮
		}
