...
@Namespace("/demo")
@Action(value = "assessAction", results = {
		@Result(name = "assessList", location = "/demo/weixin/assess.jsp") })
public class AssessAction extends BaseAction<Assess> {
	 ...
	 * 序列号
	 */
	private static final Logger log = Logger.getLogger(AssessAction.class);
	...
	public void datagrid() throws Exception {
		...
		model.setOrder("desc");
		super.writeJson(assessService.dataGrid(model));
	}
	...
	public void edit() {
		Json json = new Json();
		try {
			Assess assess = assessService.update(model);
	...
	public void delete() {
		Json json = new Json();
		try {
			assessService.delete(model.getIds());
			json.setSuccess(true);
			json.setMsg("删除数据成功！");
		} catch (Exception e) {
			json.setMsg("删除数据失败！");
